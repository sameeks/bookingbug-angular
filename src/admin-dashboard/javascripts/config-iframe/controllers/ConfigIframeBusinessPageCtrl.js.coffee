...
.controller 'ConfigIframeBusinessPageCtrl',['$scope', '$state', '$rootScope', ($scope, $state, $rootScope) ->
  $scope.pageHeader = 'CONFIG_PAGE.BUSINESS.TITLE'

  $scope.tabs = [
    {
      name: 'CONFIG_PAGE.BUSINESS.TAB_STAFF',
      icon: 'fa fa-male',
      path: 'config.business.page({path: "person"})'
    },
    {
      name: 'CONFIG_PAGE.BUSINESS.TAB_RESOURCES',
      icon: 'fa fa-diamond',
      path: 'config.business.page({path: "resource"})'
    },
    {
      name: 'CONFIG_PAGE.BUSINESS.TAB_SERVICES',
      icon: 'fa fa-wrench',
      path: 'config.business.page({path: "service"})'
    },
    {
      name: 'CONFIG_PAGE.BUSINESS.TAB_WHO_WHAT_WHERE',
