...
.controller 'SettingsIframeIntegrationsPageCtrl',['$scope', '$state', '$rootScope', ($scope, $state, $rootScope) ->
  $scope.pageHeader = 'SETTINGS_PAGE.INTEGRATIONS.TITLE'

  $scope.tabs = [
    {
      name: 'SETTINGS_PAGE.INTEGRATIONS.TAB_PAYMENT',
      icon: 'fa fa-credit-card',
      path: 'settings.integrations.page({path: "conf/addons/payment"})'
    },
    {
      name: 'SETTINGS_PAGE.INTEGRATIONS.TAB_ACCOUNTING',
      icon: 'fa fa-pencil-square-o',
      path: 'settings.integrations.page({path: "conf/addons/accounting"})'
    },
    {
      name: 'SETTINGS_PAGE.INTEGRATIONS.TAB_OTHER',
      icon: 'fa fa-question',
