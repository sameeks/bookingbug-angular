...
.controller 'SettingsIframeSubscriptionPageCtrl',['$scope', '$state', '$rootScope', ($scope, $state, $rootScope) ->
  $scope.pageHeader = 'SETTINGS_PAGE.SUBSCRIPTION.TITLE'

  $scope.tabs = [
    {
      name: 'SETTINGS_PAGE.SUBSCRIPTION.TAB_STATUS',
      icon: null,
      path: 'settings.subscription.page({path: "subscription/show"})'
    },
    {
      name: 'SETTINGS_PAGE.SUBSCRIPTION.TAB_PAYMENT_HISTORY',
      icon: null,
      path: 'settings.subscription.page({path: "payment_event"})'
    },
    {
      name: 'SETTINGS_PAGE.SUBSCRIPTION.TAB_INVOICES',
      icon: null,
