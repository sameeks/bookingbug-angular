...
angular.module('BB.Controllers').controller('Purchase', function($scope,  $rootScope,
  ...
  $scope.init = function(options) {
    ...
    loader.notLoaded();
    if (options.move_route) { $scope.move_route = options.move_route; }
    if (options.move_all) { $scope.move_all = options.move_all; }
    if (options.fail_msg) { $scope.fail_msg = options.fail_msg; }
    ...
    if ($scope.bb.total) {
      ...
      if ($scope.purchase.confirm_messages) { $scope.messages = $scope.purchase.confirm_messages; }
      if (!$scope.cancel_reasons) { $scope.cancel_reasons = $scope.bb.cancel_reasons; }
      if (!$scope.move_reasons) { $scope.move_reasons = $scope.bb.move_reasons; }
  ...
  var checkIfMoveBooking = function(bookings) {
    ...
    if (matches) { id = parseInt(matches[1]); }
    if (id) {
      let move_booking = (Array.from(bookings).filter((b) => b.id === id).map((b) => b));
      if ((move_booking.length > 0) && $scope.isMovable(bookings[0])) { return $scope.move(move_booking[0]); }
    }
  };
  ...
  var getCompanyID = function() {
    ...
    if (matches) { company_id = matches[1]; }
    return company_id;
  };
  ...
  var getPurchaseID = function() {
    ...
    if (matches) {
      ...
      if (QueryStringService('ref')) { id = QueryStringService('ref'); }
    }
    if (QueryStringService('booking_id')) { id = QueryStringService('booking_id'); }
    return id;
  };
  ...
  $scope.move = function(booking, route, options) {
    ...
    if (!route) { route = $scope.move_route; }
    if ($scope.move_all) {
    ...
    return PurchaseService.bookWaitlistItem(params).then(function(purchase) {
      ...
      return $scope.purchase.$getBookings().then(function(bookings) {
        ...
        $scope.waitlist_bookings = ((() => {
          let result = [];
          for (booking of Array.from($scope.bookings)) {             if (booking.on_waitlist && (booking.settings.sent_waitlist === 1)) {
              result.push(booking);
            }
          }
          return result;
  ...
  $scope.delete = function(booking) {
    ...
    let modalInstance = $uibModal.open({
      ...
      resolve: {
        ...
        cancel_reasons() { return $scope.cancel_reasons; }
      }
    });

    return modalInstance.result.then(function(booking) {
      let cancel_reason = null;
      if (booking.cancel_reason) { ({ cancel_reason } = booking); }
  ...
  $scope.deleteAll = function() {
    ...
    let modalInstance = $uibModal.open({
      ...
      resolve: {
        purchase() { return $scope.purchase; }
      }
    });
  ...
  var setMoveReasonsToBB = function() {
    ...
    if ($scope.move_reasons) { return $scope.bb.move_reasons = $scope.move_reasons; }
  };

  return setCancelReasonsToBB = function() {
    
    if ($scope.cancel_reasons) { return $scope.bb.cancel_reasons = $scope.cancel_reasons; }
  };
});
