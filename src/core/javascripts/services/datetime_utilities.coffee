angular.module('BB.Services').factory "DateTimeUtilitiesService", () ->
  ...
  checkDefaultTime: (date, time_slots, basket_item, config) ->
    ...
    ((basket_item.defaults.person and basket_item.defaults.person.self is basket_item.person.self) or _.isBoolean(basket_item.person)) and 
      ((basket_item.defaults.resource and basket_item.defaults.resource.self is basket_item.resource.self) or _.isBoolean(basket_item.resource) or !config.item_defaults.merge_resources))
