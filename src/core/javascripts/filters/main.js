...
angular.module('BB.Filters').filter('address_multi_line', () =>
  address => {
    ...
    let str = "";
    if (address.address1) { str += address.address1; }
    if (address.address2 && (str.length > 0)) { str += "<br/>"; }
    if (address.address2) { str += address.address2; }
    if (address.address3 && (str.length > 0)) { str += "<br/>"; }
    if (address.address3) { str += address.address3; }
    if (address.address4 && (str.length > 0)) { str += "<br/>"; }
    if (address.address4) { str += address.address4; }
    if (address.address5 && (str.length > 0)) { str += "<br/>"; }
    if (address.address5) { str += address.address5; }
    if (address.postcode && (str.length > 0)) { str += "<br/>"; }
    if (address.postcode) { str += address.postcode; }
    return str;
  }
);
...
angular.module('BB.Filters').filter('distance', ($translate, bbLocale) =>
  function(distance_in_km, round_by) {
    ...
    if (!distance_in_km) { return ''; }

    let distance  = distance_in_km;
    let use_miles = bbLocale.getLocale().match(/^(en|en-gb|en-us)$/gi);
    if (use_miles) { distance  *= 0.621371192; }
    if (round_by) { distance  = Math.round(distance * Math.pow(10, round_by)) / Math.pow(10, round_by); } 
...
angular.module('BB.Filters').filter('currency', ($window, $rootScope, CompanyStoreService, $translate) =>
  function(amount, currency_code, pretty_price) {
    ...
    if (pretty_price == null) { pretty_price = false; }
    if (!angular.isNumber(amount)) { return; }

    let currency_codes = {USD: "$", GBP: "£", AUD: "$", EUR: "€", CAD: "$", MIXED: "~", RUB: "₽"};

    if (!currency_code) { ({ currency_code } = CompanyStoreService); }
...
angular.module('BB.Filters').filter('pretty_price', ($translate, $filter) =>
  function(price, currency_code) {
    if (parseFloat(price) === 0) { return $translate.instant('CORE.FILTERS.PRETTY_PRICE.FREE');
    } else { return $filter('currency')(price, currency_code, true); }
  }
);
...
angular.module('BB.Filters').filter('twelve_hour_time', $window =>
  function(time, options) {
    ...
    if (!angular.isNumber(time)) { return; }
...
angular.module('BB.Filters').filter('local_phone_number', (CompanyStoreService, ValidatorService) =>
  function(phone_number, country_code) {
    ...
    if (!country_code) { ({ country_code } = CompanyStoreService); }

    switch (country_code) {
      case "gb": return phone_number.replace(/^(\+44 \(0\)|\S{0})/, '0');
      case "us": return phone_number.replace(ValidatorService.us_phone_number, "($1) $2 $3");
      default:
        return phone_number;
    }
  }
