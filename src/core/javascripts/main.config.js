...
angular.module('BB').config(function($locationProvider, $httpProvider, $provide, ie8HttpBackendProvider, uiGmapGoogleMapApiProvider) {
  ...
  let lowercase = function(string) {
    if (angular.isString(string)) { return string.toLowerCase(); } else { return string; }
  };
  ...
  if (result) { webkit = parseFloat(result[1]); }
...
window.bookingbug = {
  logout(options) {
    if (!options) { options = {}; }
    if (options.reload !== false) { options.reload = true; }
    let logout_opts = {
      app_id: 'f6b16c23',
      app_key: 'f0bc4f65f4fbfe7b4b3b7264b655f5eb'
    };
    if (options.root) { logout_opts.root = options.root; }
