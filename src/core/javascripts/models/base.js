...
angular.module('BB.Models').service("BaseModel", ($q, $injector, $rootScope, $timeout) =>
  ...
  class Base {
        ...
        return (() => {
          ...
          for (let link in links) {
            ...
            result.push(((link, obj, name) => {
              if (!this[name]) {
                this[name] = function(options) { return this.$buildOject(link, options); };
              }
              if (!this[`$${name}`]) {
                return this[`$${name}`] = function(options) { return this.$buildOjectPromise(link, options); };
              }
            })(link, obj, name));
          }
          return result;
    ...
    $buildOject(link, options) {
      ...
      if (this.__linkedData[linkId]) { return this.__linkedData[linkId]; }
    ...
    $buildOjectPromise(link, options) {
      ...
      if (this.__linkedPromises[linkId]) { return this.__linkedPromises[linkId]; }
      let prom = $q.defer();
    ...
    $href(rel, params) {
      if (this._data) { return this._data.$href(rel, params); }
    }

    $has(rel) {
      if (this._data) { return this._data.$has(rel); }
    }

    $flush(rel, params) {
      if (this._data) { return this._data.$flush(rel, params); }
    }

    $get(rel, params) {
      if (this._data) { return this._data.$get(rel, params); }
    }

    $post(rel, params, dat) {
      if (this._data) { return this._data.$post(rel, params, dat); }
    }

    $put(rel, params, dat) {
      if (this._data) { return this._data.$put(rel, params, dat); }
    }

    $patch(rel, params, dat) {
      if (this._data) { return this._data.$patch(rel, params, dat); }
    }

    $del(rel, params, dat) {
      if (this._data) { return this._data.$del(rel, params, dat); }
    }

    $links() {
      if (this._data) { return this._data.$links(); }
    }

    $link(rel) {
      if (this._data) { return this._data.$link(rel); }
    }

    $toStore() {
      if (this._data) { return this._data.$toStore(); }
    }
  }
);
