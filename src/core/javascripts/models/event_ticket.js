  ...
  class EventTicket extends BaseModel {
    ...
    constructor(data) {
      ...
      if (this.max_spaces) {
        ...
        if (this.counts_as) { ms = this.max_spaces / this.counts_as; }
        if (ms < max) {
          this.max = ms;
        }
      }
    ...
    */
    getRange(event, cap) {
      ...
      let pool = null;
      if (this.ticket_set) { pool = this.pool_id; }
    ...
    */
    getMax(ev, pool, cap) {
      ...
      for (let ticket of Array.from(ev.tickets)) {
        if ((ticket.pool_id === this.pool_id) || wait_spaces) { used += ticket.totalQty(); }
      }
      ...
      if (live_max < 0) { live_max = 0; }

      var left = left - used;
      if (left < 0) { left = 0; }

      // use ticket cap if set
      if (this.cap) { ({ cap } = this); }
      ...
      if (cap) {
        let c = cap;
        if (this.counts_as) { c = cap / this.counts_as; }
        if (c < live_max) {
          return c;
        }
      }
    ...
    */
    totalQty() {
      ...
      if (!this.counts_as) { return this.qty; }
    ...
    */
    add(value) {
      ...
      if ((angular.isNumber(this.qty) && ((this.qty >= this.max) && (value > 0))) || ((this.qty === 0) && (value < 0))) { return; }
