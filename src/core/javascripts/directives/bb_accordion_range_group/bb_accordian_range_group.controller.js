...
angular.module('BB.Controllers').controller('AccordionRangeGroup', function($scope, $attrs, $rootScope, $q, FormDataStoreService, GeneralOptions, DateTimeUtilitiesService, $translate, CompanyStoreService) {
  ...
  var setData = function() {
    ...
    if ($scope.slots) {
      ...
      angular.forEach($scope.slots, function(slot) {
        ...
        if ((slot_time >= $scope.start_time) && (slot_time < $scope.end_time) && (slot.avail === 1)) { return $scope.accordion_slots.push(slot); }
      });
  ...
  */
  var updateAvailability = function(day, slot) {
    ...
    if ($scope.accordion_slots) { $scope.has_availability = hasAvailability(); }
    ...
    if (day && slot) {
      ...
      if (day.date.isSame($scope.day.date) && (slot_time >= $scope.start_time) && (slot_time < $scope.end_time)) { $scope.selected_slot = slot; }
    ...
    if ($scope.selected_slot) {
      ...
      if ($scope.options.collapse_when_time_selected) { return $scope.is_open = false; }
    } else {
  ...
  */
  var hasAvailability = function() {
    if (!$scope.accordion_slots) { return false; }
