          ...
          switch (data.type) {
            case "load":
              return scope.$apply(() => {
                ...
                if (data.custom_partial_url) { applyCustomPartials(event.data.custom_partial_url, scope, element); }
                if (data.custom_stylesheet) { applyCustomStylesheet(data.custom_stylesheet); }
                if (data.scroll_offset) { return GeneralOptions.scroll_offset = data.scroll_offset; }
              }
              );
          }
        }
    ...
    if (custom_partial_url != null) {
      ...
      return $http.get(custom_partial_url).then(custom_templates =>
        $compile(custom_templates.data)(scope, function(custom, scope) {
          ...
          let custom_form = ((() => {
            let result = [];
            for (e of Array.from(custom)) {               if (e.id === 'payment_form') {
                result.push(e);
              }
            }
            return result;
  ...
  return applyCustomStylesheet = function(href) {
    ...
    if (!document.getElementById(css_id)) {
      ...
      return link.onload = function() {
        if ('parentIFrame' in $window) { return parentIFrame.size(); }
      };
    }
  };
});
