...
let BBServicesCtrl = function($scope, $rootScope, $q, $attrs, $uibModal, $document, BBModel, FormDataStoreService, ValidatorService, ErrorService, $filter, LoadingService) {
  ...
  if ($attrs.bbItem) { $scope.booking_item = $scope.$eval($attrs.bbItem); }
  if ($attrs.bbShowAll || $scope.options.show_all) { $scope.show_all = true; }
  if ($scope.options.allow_single_pick) { $scope.allowSinglePick = true; }
  if ($scope.options.hide_disabled) { $scope.hide_disabled = true; }
  ...
  $scope.init = function(comp) {
    ...
    if (($scope.booking_item.person && !$scope.booking_item.anyPerson()) || ($scope.booking_item.resource && !$scope.booking_item.anyResource())) {
      ...
      let ispromise = BBModel.BookableItem.$query({company: $scope.bb.company, cItem: $scope.booking_item, wait: ppromise, item: 'service'});
  ...
  */
  $scope.selectItem = (item, route, options) => {
    ...
    if ($scope.$parent.$has_page_control) {
      ...
      if (options.skip_step) { $scope.skipThisStep(); }
