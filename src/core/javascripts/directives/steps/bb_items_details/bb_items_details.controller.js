angular.module('BB.Controllers').controller('ItemDetails', function($scope, $attrs, $rootScope, PurchaseBookingService, AlertService, BBModel, FormDataStoreService, ValidatorService, $uibModal, $document, $translate, $filter, GeneralOptions, PurchaseService, LoadingService) {
  ...
  $rootScope.connection_started.then(function() {
    if (!confirming) { return $scope.loadItem($scope.bb.current_item); }
  }
  ...
  */
  $scope.loadItem = function(item) {
    ...
    confirming = true;
    $scope.item = item;
    if ($scope.bb.private_note) { $scope.item.private_note = $scope.bb.private_note; }
    ...
    if ($scope.item.item_details) {
      ...
      if ($scope.bb.item_defaults.answers) { BBModel.Question.$addAnswersFromDefaults($scope.item_details.questions, $scope.bb.item_defaults.answers); }
      $scope.recalc_price();
      loader.setLoaded();
  ...
  */
  $scope.recalc_price = function() {
    ...
    if (!bprice) { bprice = $scope.item.price; }
  ...
  */
  $scope.confirm = function(form, route) {
    ...
    if ($scope.$parent.$has_page_control) { return true; }
  ...
  */
  $scope.confirm_move = function(route) {
    ...
    confirming = true;
    if (!$scope.item) { $scope.item = $scope.bb.current_item; }
        ...
        return PurchaseBookingService.update($scope.item).then(function(booking) {
          ...
          if ($scope.bb.purchase) {
            for (let _i = 0; _i < $scope.bb.purchase.bookings.length; _i++) {
              ...
              if (oldb.id === b.id) { $scope.bb.purchase.bookings[_i] = b; }
            }
          }
