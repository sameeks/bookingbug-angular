angular.module('BB.Controllers').controller('TimeRangeListStackedController', function(
  ...
  */
  $scope.highlightSlot = function(slot, day) {
    ...
    if (day && slot && (slot.availability() > 0)) {
      ...
      if ($scope.selected_slot) { $scope.selected_slot.selected = false; }
  ...
  $scope.loadData = function() {
    ...
    return $q.all(pslots).then(function(res) {
      ...
      for (let _i = 0; _i < grouped_items.length; _i++) {
        ...
        let slots = res[_i];
        if (!slots || (slots.length === 0)) { $scope.data_valid = false; }
  ...
  */
  var spliceExistingDateTimes = function(stacked_item, slots) {
    ...
    if (!stacked_item.datetime && !stacked_item.date) { return; }
