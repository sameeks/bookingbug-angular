angular.module('BB.Controllers').controller('TimeRangeList',
    function ($scope, $element, $attrs, $rootScope, $q, AlertService, LoadingService, BBModel, FormDataStoreService,
              DateTimeUtilitiesService, SlotDates, viewportSize, ErrorService) {
...
// TODO make this advance to the next month
        ...
        $scope.showFirstAvailableDay = () =>
            SlotDates.getFirstDayWithSlots($scope.data_source, $scope.selected_day).then(
                function (day) {
                    $scope.no_slots_in_week = false;
                    setTimeRange(day);
                    return $scope.loadData();
