angular.module('BB.Controllers').controller('Login', function($scope, $rootScope, $q, $location, LoginService, ValidatorService, AlertService, LoadingService, BBModel) {
  ...
  */
  $scope.login_sso = (token, route) =>
    $rootScope.connection_started.then(() => {
      return LoginService.ssoLogin({company_id: $scope.bb.company.id, root: $scope.bb.api_url}, {token}).then(member => {
        if (route) { return $scope.showPage(route); }
      }
  ...
  */
  $scope.sendPasswordReset = email =>
    LoginService.sendPasswordReset($scope.bb.company, {email, custom: true}).then(() => AlertService.raise('PASSWORD_RESET_REQ_SUCCESS')
    , err => {
  ...
  */
  return $scope.updatePassword = function(new_password, confirm_new_password) {
    ...
    if ($rootScope.member && new_password && confirm_new_password && (new_password === confirm_new_password)) {
      return LoginService.updatePassword($rootScope.member, {new_password, confirm_new_password, persist_login: $scope.login_form.persist_login}).then(member => {
        if (member) {
