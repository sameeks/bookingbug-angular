...
let BBAdminCalendarCtrl = function($scope, $element, $controller, $attrs, BBModel, $rootScope) {
  ...
  $scope.initialise = function() {
    ...
    if ($scope.bb.current_item.person) { $scope.person_name   = $scope.bb.current_item.person.name; }
    if ($scope.bb.current_item.resource) { return $scope.resource_name = $scope.bb.current_item.resource.name; }
  };
...
var BBAdminCalendarConflictCtrl = function($scope, $element, $controller, $attrs, BBModel) {
    ...
    let getCurrentPersonId = function() {
      if ($scope.bb.current_item.person != null) { return $scope.bb.current_item.person.id; }
      if ($scope.bb.current_item.defaults.person != null) { return $scope.bb.current_item.defaults.person.id; }
    };
    ...
    BBModel.Admin.Booking.$query(params).then(function(bookings) {
      if (bookings.items.length > 0) {
        ...
        if ($scope.nearby_bookings.length === 0) { $scope.nearby_bookings = false; }
        if ($scope.overlapping_bookings.length === 0) { $scope.overlapping_bookings = false; }
      }
