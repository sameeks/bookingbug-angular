      ...
      if (!member.$has('bookings')) {
        ...
        member.$get('bookings', params).then(bookings => {
          ...
          if (angular.isArray(bookings)) {
            ...
            bookings = ((() => {
              let result = [];
              for (booking of Array.from(bookings)) {                 result.push(new BBModel.Member.Booking(booking));
              }
              return result;
            })());
            ...
            return bookings.$get('bookings', params).then(bookings => {
              bookings = ((() => {
                let result1 = [];
                for (booking of Array.from(bookings)) {                   result1.push(new BBModel.Member.Booking(booking));
                }
                return result1;
              })());
              return deferred.resolve(bookings);
