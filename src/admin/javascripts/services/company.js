...
angular.module('BBAdmin.Services').factory('AdminCompanyService', (
  ...
  ({
    query(params) {
      ...
      if (!$rootScope.bb.api_url) { $rootScope.bb.api_url = $sessionStorage.getItem("host"); }
      if (!$rootScope.bb.api_url) { $rootScope.bb.api_url = params.apiUrl; }
      if (!$rootScope.bb.api_url) { $rootScope.bb.api_url = ""; }
